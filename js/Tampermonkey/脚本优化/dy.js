...
(function () {
  ...
  window.onload = function () {
    ...
    videoButton.addEventListener('click', () => {
      ...
      const video = videos[rules[len]]
      index = rules[len]
      const src = video?.firstChild?.src || video?.src || null
      const date = new Date()
      let author = document.querySelectorAll(authorSelector[0])[index]?.textContent || document.querySelectorAll(authorSelector[1])[index]?.textContent
        || document.querySelector(authorSelector[2])?.textContent || ''
      author = author.slice(1, author.length)
      const time = document.querySelector(createTime)?.textContent.slice(2) || `${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}_${date.getHours()}_${date.getMinutes()}_${date.getSeconds()}`
      const filename = `${author}-${time}.mp4`
      console.warn('视频', author, src)
      if (!src) {
        dy.notice('获取url地址失败')
      }
